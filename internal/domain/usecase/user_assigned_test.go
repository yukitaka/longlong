...
func TestNewUserAssigned(t *testing.T) {
	...
	members := &[]entity.OrganizationMember{*entity.NewOrganizationMember(organ, &(*individuals)[0], value_object.MEMBER)}
	...
	itr := NewUserAssigned(1, individualRep, organizationRep, memberRep)
	organizations, _ := itr.OrganizationList()
